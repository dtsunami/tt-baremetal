...
mode=sys.argv[1] if len(sys.argv)>1 else "max"   # 'max'=0xffffffff (never gate) | 'zero'=0
val=0xFFFFFFFF if mode=="max" else 0
...
coord.noc_write32(CG0,val); coord.noc_write32(CG1,val)
pr(f"[cg-{mode}] CG_CTRL_HYST0=0x{coord.noc_read32(CG0):08x} HYST1=0x{coord.noc_read32(CG1):08x}")
def relerr(dev,g):
...
pr(f"\n[cg-{mode}] {npass}/10 rings PASS => {'CLOCK-GATING DISABLE FIXES IT!!!' if npass==10 else 'no effect'}")
