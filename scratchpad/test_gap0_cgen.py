...
pr(f"[cgen] CG_CTRL_EN before = 0x{coord.noc_read32(CG_CTRL_EN):08x}")
coord.noc_write32(CG_CTRL_EN, 0)     # *** master clock-gating DISABLE ***
pr(f"[cgen] CG_CTRL_EN after  = 0x{coord.noc_read32(CG_CTRL_EN):08x}  (wrote 0)")
def relerr(dev,g):
...
pr(f"\n[cgen] {npass}/12 rings PASS => {'*** CG_CTRL_EN=0 FIXES IT — clock-gating sil bug!! ***' if npass==12 else 'still stalls'}")
