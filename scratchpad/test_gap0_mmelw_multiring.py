"""GAP-0 ISOLATION #1b: drive resident_mm_elw_perf MULTI-RING (E=(A@B)*D) — eltwise binary, NO reciprocal.

backward (matmul+eltwise+recip) STALLS multi-ring. mm_elw isolates: matmul + eltwise-binary ELWMUL, NO
MREC/reciprocal, NO forward SFPU. So:
  - if mm_elw SURVIVES multi-ring -> the RECIPROCAL (MREC) is the sole accumulator (clean target).
  - if mm_elw STALLS multi-ring    -> the eltwise-binary itself accumulates.

Run: /home/starboy/bhtop/.venv/bin/python scratchpad/test_gap0_mmelw_multiring.py [NRINGS]
"""
import sys, time
sys.path.insert(0, "/home/starboy/bhtop/src")
from ttexalens import init_ttexalens
from ttexalens.tt_exalens_lib import read_word_from_device as rd, read_words_from_device as rds, write_words_to_device as wr
from bhtop.tensix.loader import TensixLauncher
from bhtop.tensix import matmul as MM, llk_run
from bhtop.tensix.resident import boot_resident

NRINGS = int(sys.argv[1]) if len(sys.argv) > 1 else 3
DB, DONE, HB, DBG_U, DBG_M, DBG_P = 0x16000, 0x16010, 0x16020, 0x16030, 0x16040, 0x16050
A_ADDR, B_ADDR, D_ADDR, OUT = 0x21000, 0x31000, 0x61000, 0x51000
POISON = 0xBADF00D5
enc = lambda flat: MM.pack_bf16_words([float(x) for x in MM.tilize32(flat)])
dec = lambda a, ctx, c: MM.untilize32(MM.unpack_bf16_words(rds(c, a, word_count=512, context=ctx)))


def snap(rdbg):
    def one(h):
        try:
            if h.is_in_reset(): return "reset"
            pc = h.get_pc(); time.sleep(0.03); pc2 = h.get_pc()
            return f"pc=0x{pc:x} {'STUCK' if pc == pc2 else 'advancing'}"
        except Exception as e: return f"err:{e}"
    return {c: one(rdbg[c]) for c in ("UNPACK", "MATH", "PACK")}


def main():
    pr = lambda *a: print(*a, flush=True)
    ctx = init_ttexalens()
    coord = TensixLauncher.at(1, 2, ctx=ctx).coord
    pr(f"[mmelw-mr] driving {NRINGS} rings on ONE boot (matmul + ELWMUL, no recip)")

    A = [((i * 7 + k * 3) % 13) * 0.1 for i in range(32) for k in range(32)]   # flat 1024, row-major
    B = [((k * 5 + j * 2) % 11) * 0.1 for k in range(32) for j in range(32)]
    D = [((i + j) % 7) * 0.1 + 0.05 for i in range(32) for j in range(32)]
    C1 = MM.matmul_golden(A, B)
    Eg = [C1[idx] * D[idx] for idx in range(1024)]   # elementwise (A@B) * D

    ov = {"ELTWISE_BINARY_OP": "constexpr auto ELTWISE_BINARY_OP = ckernel::EltwiseBinaryType::ELWMUL;"}
    b = llk_run.build("resident_mm_elw_perf", run_type="L1_TO_L1", fidelity="HiFi4", fp32_acc=False,
                      formats=None, overrides=ov)
    assert b["ok"], b["log"][-2000:]
    rdbg = boot_resident("resident_mm_elw_perf", coord, ctx=ctx,
                         runtime_words=[1, 1, 1, 1, 1, 128, 128, 0, 4, 4], clear_words=48)
    time.sleep(0.3)
    wr(coord, A_ADDR, enc(A), context=ctx)
    wr(coord, B_ADDR, enc(B), context=ctx)
    wr(coord, D_ADDR, enc(D), context=ctx)
    pr("[mmelw-mr] booted + staged once\n")

    def relerr(dev, gold):
        n = sum(abs(dev[i] - gold[i]) for i in range(1024)); d = sum(abs(x) for x in gold) + 1e-9
        return n / d

    npass = 0
    for r in range(1, NRINGS + 1):
        wr(coord, OUT, [POISON] * 512, context=ctx)
        wr(coord, DB, [r], context=ctx)
        t0 = time.time()
        while time.time() - t0 < 4.0 and rd(coord, DONE, context=ctx) != r: time.sleep(0.004)
        if rd(coord, DONE, context=ctx) != r:
            pr(f"[ring {r}] *** STALL *** DONE={rd(coord,DONE,context=ctx)} DBG_U=0x{rd(coord,DBG_U,context=ctx):x} "
               f"DBG_M=0x{rd(coord,DBG_M,context=ctx):x} DBG_P=0x{rd(coord,DBG_P,context=ctx):x}  {snap(rdbg)}")
            pr("[mmelw-mr] stop. recover: tt-smi -r 0"); break
        e = relerr(dec(OUT, ctx, coord), Eg); ok = e < 0.05; npass += ok
        pr(f"[ring {r}] DONE in {time.time()-t0:.2f}s  rel-err={e:.2e} -> {'PASS' if ok else 'CHECK'}")

    pr(f"\n[mmelw-mr] {npass}/{NRINGS} rings PASS => "
       f"{'eltwise-binary IS multi-ring OK -> RECIPROCAL is the accumulator' if npass==NRINGS else 'eltwise-binary ITSELF stalls multi-ring'}")
    return npass == NRINGS


if __name__ == "__main__":
    sys.exit(0 if main() else 1)
