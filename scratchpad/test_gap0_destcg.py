...
pr(f"[destcg] DEST_CG_CTRL before = 0x{coord.noc_read32(DEST_CG_CTRL):08x}")
coord.noc_write32(DEST_CG_CTRL, 0)   # *** the LLK 'ugly hack': disable DEST clock gating ***
pr(f"[destcg] DEST_CG_CTRL after  = 0x{coord.noc_read32(DEST_CG_CTRL):08x}  (wrote 0)")
def relerr(dev,g):
...
pr(f"\n[destcg] {npass}/12 rings PASS => {'*** DEST_CG_CTRL=0 FIXES IT — clock-gating sil bug, chicken-bit workaround! ***' if npass==12 else 'still stalls'}")
