...
DBG_FEATURE_DISABLE=0xFFB12068
...
# *** THE CHICKEN BIT: set DBG_FEATURE_DISABLE bit 11 (the documented BH sil-bug workaround) ***
coord.noc_write32(DBG_FEATURE_DISABLE, 1<<11)
pr(f"[chicken] set DBG_FEATURE_DISABLE bit11 -> 0x{coord.noc_read32(DBG_FEATURE_DISABLE):08x}")
def relerr(dev,g): 
    n=sum(abs(dev[i]-g[i]) for i in range(1024)); d=sum(abs(x) for x in g)+1e-9; return n/d
npass=0
for r in range(1,11):
    wr(coord,OUT,[0xBADF00D5]*512,context=ctx)
    ...
    if rd(coord,DONE,context=ctx)!=r:
        pr(f"[ring {r}] *** STALL ***"); break
    e=relerr(dec(OUT,ctx,coord),Eg); ok=e<0.05; npass+=ok
    pr(f"[ring {r}] DONE rel-err={e:.2e} -> {'PASS' if ok else 'CHECK'}")
pr(f"\n[chicken] {npass}/10 rings PASS with DBG_FEATURE_DISABLE bit11 "
   f"=> {'CHICKEN BIT FIXES IT!!! (sil-bug workaround)' if npass==10 else 'no effect / still stalls'}")
